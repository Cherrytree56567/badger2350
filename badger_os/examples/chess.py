...
def reset_saved_state():
    try:
        ...
        print("Saved game state has been reset (file deleted).")
    except OSError:
        print("No saved game state file found. Nothing to delete.")
    
    # Reinitialize the game state (game history)
    hist = [Position(initial, 0, (True, True), (True, True), 0, 0)]
    save_game_state(hist)  # Optionally save the fresh state to persistent storage.
    print("Game state has been reinitialized.")


def save_game_state(state):
    """
    Save the game state to a file. The state can be a dictionary that holds:
      - current board
      - game history (e.g., list of moves or board states)
      - any additional variables needed to resume the game.
    """
    try:
...
def load_game_state():
    """
    Attempt to load the game state from persistent storage.
...
def update_state(hist):
    # For simplicity, you might save only the most recent board state.
    state = {
        "history": [h.board for h in hist],
        "current_move": hist[-1].board,
